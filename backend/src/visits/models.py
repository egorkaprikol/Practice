...
class Visit(Base):
    ...
    place: Mapped[int] = mapped_column(ForeignKey("places.id"), nullable=False, index=True)
    date: Mapped[datetime] = mapped_column(insert_default=func.now(), index=True)
    doctor: Mapped[int] = mapped_column(ForeignKey("doctors.id"), nullable=False, index=True)
    patient: Mapped[int] = mapped_column(ForeignKey("patients.id"), nullable=False, index=True)
